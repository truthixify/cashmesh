...
| PostgreSQL repository integration | `pnpm test:integration` | Migration, invoice/request atomicity, Cashu evidence, reservation lifecycle, encrypted custody, Stellar quote ownership, fresh melt authorization, ambiguity retention, restart, concurrency, corruption, and rollback behavior |
...
| Cashu mint HTTP integration | Not implemented | Keyset, proof-state, and Stellar quote/melt network behavior is covered by mocked transport; no live mint is called |
